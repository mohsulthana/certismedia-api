...
function create_reporting($id_user, $token, $rollUp='hour')
{
    ...
    $body = '{
        ...
        "format": "csv",
        "metrics": [
          "other_conversions","impressions","clicks","views","completed_views","ctr"
        ],
        "rollUp": "'.$rollUp.'",
        "startDate": "'.$startDate.'"
      }';
...
function importCSV($path_file, $table, $hourly=true)
{
    $now = date("Y-m-d");
    // return var_dump($now)
    if(true) {
        $user = 'fykfaumy_root';
        $p = 'mysql fykfaumy_reporting --user='.escapeshellarg($user).'  --password='.escapeshellarg($user).' -e "DELETE FROM '.$table.'  WHERE date_time LIKE '."'".$now."%' ".'"';
        shell_exec($p);
    }
    // var_dump($p);
    // return;
...
function test_importCSV($path_file, $table, $hourly=true)
{
    $now = date("Y-m-d");
    $now = '2021-07-05';
    var_dump($now);
    ...
    // return;
    $user = 'fykfaumy_root';
    $p = 'mysql fykfaumy_reporting --user='.escapeshellarg($user).'  --password='.escapeshellarg($user).' -e "LOAD DATA LOCAL INFILE '.escapeshellarg($path_file)."REPLACE INTO TABLE ".$table." FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '\\\"' LINES TERMINATED BY '\\n' IGNORE 1 ROWS\"";
    var_dump($p);
    // return;
    shell_exec($p);
}
