...
class Reporting_model extends Model {
  ...
  protected $allowedFields = ['campaign_id','campaign_name', 'creative_id', 'creative_name', 'creative_size', 'inventory_id', 'inventory_name', 'exchange_name', 'other_conversions','impressions', 'views', 'completed_views', 'clicks', 'ctr', 'date_time'];
  
  public function getCampaignNameById($table, $campaign_id) {
        
    $query = 'SELECT campaign_id,campaign_name FROM '.$table;
    $query .= ' WHERE campaign_id='.$campaign_id .' GROUP BY campaign_id';
  ...
  public function getDataByCampaignId($table, $condition=null, $field=null) {
    ...
    $query = 'SELECT '.$field.',sum(`other_conversions`),sum(`impressions`) as impressions,sum(`clicks`) as clicks,sum(`views`) as views,sum(`completed_views`) as completed_views,sum(`ctr`) as ctr,`date_time` FROM (SELECT '.$field.',`impressions`,`clicks`,`views`,`completed_views`,`ctr`,`date_time` FROM '.$table;
    if ($condition != null)
...
//     $query = 'SELECT '.$field.',sum(`impressions`) as impressions,sum(`clicks`) as clicks,sum(`views`) as views,sum(`completed_views`) as completed_views,sum(`ctr`) as ctr FROM '.$table;
//     $query .= ' WHERE campaign_id='.$campaign_id;
//     $query .= ' GROUP BY inventory_name';
...
//   }
  ...
  public function getDataDashboard($table, $condition=null, $field=null) {
    ...
    $query = 'SELECT '.$field.',sum(`other_conversions`),sum(`impressions`) as impressions,sum(`clicks`) as clicks,sum(`views`) as views,sum(`completed_views`) as completed_views,sum(`clicks`)/sum(`impressions`) * 100 as ctr FROM '.$table;
    $query .= ' GROUP BY campaign_id';
    return $this->db->query($query)->getResultArray();
  }
  
  public function getDataDaily($table, $campaign_id, $condition=null) {
    $query = 'SELECT date_time,sum(`other_conversions`),sum(`impressions`) as impressions,sum(`clicks`) as clicks,sum(`views`) as views,sum(`completed_views`) as completed_views, sum(`clicks`)/sum(`impressions`) * 100 as ctr FROM '.$table;
    $query .= ' WHERE campaign_id='.$campaign_id;
    if ($condition != null)
        $query .= ' AND '.$condition;
  ...
  public function getDataAdSize($table, $campaign_id, $condition=null) {
    $field = 'creative_size';
    $query = 'SELECT '.$field.',sum(`other_conversions`),sum(`impressions`) as impressions FROM '.$table;
    $query .= ' WHERE campaign_id='.$campaign_id;
    if ($condition != null)
        $query .= ' AND '.$condition;
  ...
  public function getAllReportData($account_id)
  {
    ...
    $sql = 'SELECT `campaign_id`,sum(`other_conversions`) as other_conversions, SUM(`impressions`) as impression, SUM(`clicks`) as click, SUM(`views`) as view, SUM(`completed_views`) as completed_view,  SUM(`ctr`) as ctr FROM `report_csv` WHERE `account_id`=? GROUP BY `campaign_id`';
    $result = $this->db->query($sql, $account_id);
    return $result->getResultArray();
  }
